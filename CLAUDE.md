...
- **Scene timings follow the narration, not a word count.** `initializeAudioSegments` records every scene from its own text (`recordSceneNarration`) and times it to that take plus 0.6 s. It used to slice the one long recording at word-count boundaries, which put the end of each scene's sentence into the next scene, and the pauses between scenes are too inconsistent to cut on. A scene whose take fails stays `failed` and blocks the rebuild — never fill it from the long recording. Only a generator without `generateTTSAudio` still slices, scaled so the slices sum to the recording that exists. The per-scene takes cost one TTS request per scene on top of the full narration, and their timings no longer match the pipeline's first assembled video until a rebuild. Re-recording one scene (`regenerateNarration` and the regenerate path) re-times that scene to the new take plus 0.6 s through `narrationDuration`; otherwise the rebuild's `atrim` cuts the last words.
